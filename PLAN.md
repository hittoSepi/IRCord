# ircord CLI — Implementation Plan

## Goal

Create an `ircord` CLI binary that manages a running IRCord server via the admin socket. Hybrid command system: builtin C++ commands + extensible QuickJS script commands.
...
Reuse QuickJS from `ircord-plugin/`. Provide JS APIs:

```js
// Global APIs
ircord.exec(program, args)  // spawn process, return { code, stdout, stderr }
ircord.fs.read(path)        // read file
ircord.fs.write(path, data) // write file
ircord.fs.exists(path)      // check existence
...
1. CMakeLists.txt — link ircord-server-tui (AdminSocketClient), nlohmann/json
...
17. Integrate QuickJS from ircord-plugin
18. ScriptCommand wrapper (loads .js, calls execute())
19. JS API bindings (ctx.admin, ircord.exec, ircord.fs)
20. Auto-scan ~/.ircord/commands/ on startup
21. `ircord help` lists both builtin and script commands
...
26. Config file (~/.ircord/config.json) for socket path override
...
| ircord-plugin | ../ircord-plugin | QuickJS engine |
...
cmake --build build
# Produces: build/ircord(.exe)
```

## Notes

- Admin socket path: `\\.\pipe\ircord-admin` (Win) / `$XDG_RUNTIME_DIR/ircord-admin.sock` (Linux)
